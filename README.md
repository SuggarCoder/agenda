...
6. 在报表页按日期、校区、课程、班级、教师角色和归属教师查询汇总和明细。“教师汇总”展示班主任或任课老师负责班级的考勤指标，点击“查看学员明细”查看该教师范围内的全部逐人记录，支持分别导出教师汇总与明细 CSV；在每周预警页查看本周尚未出勤的学员。
...
- 教师报表按**当前班级教师分配**归属，包含其可见班级中所有学员的历史应到记录，与实际录入人无关；换老师后历史考勤随班级归属现任老师，不按历史任职区间划分责任。教师汇总仅包含筛选范围内有应到记录的教师，班级数、时段数也仅计入这些记录。
- 教师汇总按教师及角色分组，跨班学员去重，同班分别归属班主任和任课老师，各教师行不可相加作为全局总计。未分配教师的班级保留在不限定教师的全局与班级报表中。录入完成率为（实到 + 缺勤）／应到人次，应到为零时显示“—”。筛选、明细、汇总与 CSV 均受当前登录账号的班级权限限制。
